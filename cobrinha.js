...
function resgatarPontos() {
    ...
    const pontosResgatados = parseInt(score.innerText);

    // Obtém a pontuação atual do jogo
    const pontosArmazenados = parseInt(localStorage.getItem('pontosResgatados')) || 0;

    // Soma os pontos existentes com os novos pontos resgatados
    const totalPontos = pontosArmazenados + pontosResgatados;

    // Armazena o novo total de pontos localmente
    localStorage.setItem('pontosResgatados', totalPontos);
